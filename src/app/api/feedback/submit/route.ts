...
export async function POST(request: Request) {
  try {
    ...
    //
    // `userId` used to fall back to `''`. `consumer_intents.user_id` is NOT
    // NULL with an FK to `users.id` (migration 031), so an empty string is a
    // guaranteed constraint violation — swallowed by the catch below, exactly
    // the silent-write-failure shape that hid the survey-path bug. Masked
    // here only because `session.user.id` happens always to be present.
    // No id means no row: skip cleanly rather than writing a value that
    // cannot be valid.
    if (userId) {
      try {
        ...
        // NOTE: `intents` is empty when the consumer has not consented to
        // behavioural inference (gated inside extractAndPersistIntents), so
        // the alert below is consent-gated by construction — the verbatim
        // phrase never reaches a brand without it.
