...
- ✅ **CORRECTED 2026-09-23 — the `if (cronSecret && …)` fail-open is CLOSED for everything under `withCronRun`.** This entry previously read *"fails OPEN on 24 routes — publicly triggerable, including account deletion"*. That was true before `593aae9` (2026-08-13) and the fail-closed default (2026-08-17); it has been stale since. `withCronRun` (`src/lib/cron/withCronRun.ts:174-210`) checks auth **before** invoking the handler and defaults `whenUnset: 'enforce'`, so an unset `CRON_SECRET` returns **401**, not silent execution. The 24 inline checks still in those routes are **redundant, not load-bearing** — in both branches the wrapper has already decided. The wrapper also closes the `Bearer ${undefined}` variant (comparing against the literal string when the secret is unset).
- 🔴 **BUT the wrapped set is not every cron-shaped route.** `/api/social/cron` (`src/app/api/social/cron/route.ts:14`) exports `POST` directly with the inline `if (cronSecret && …)` as its **only** gate — it is genuinely fail-open. Exposure is narrower than the old wording claimed: the path is not in `PUBLIC_PREFIXES`, so middleware 401s anonymous callers; the residual hole is an **authenticated** user triggering social ingestion while `CRON_SECRET` is unset. Still exactly the shape that bites on a fresh preview env.
- ✅ **ENUMERATION FINISHED 2026-09-24 — the counts resolve, and "33 wrapped" is accurate.** `/api/cron/*` = **31** `route.ts` files, **all 31 wrapped** (`grep -L withCronRun` returns nothing); `/api/jobs/*` = **2**, both wrapped (`dsar-cleanup`, `process-deletions`). **31 + 2 = 33.** ✅ **NOTHING passes `whenUnset` anywhere** — zero occurrences across every route — so all 33 use the `'enforce'` default and fail closed. §9's "32 entries" is a **schedule** count (Vercel + cron-job.org entries), not a route count; the two numbers measure different things and were never in conflict.
- 🔴 **`/api/social/cron` remains the ONE cron-shaped route outside the wrapper** — confirmed by sweeping every `route.ts` referencing `CRON_SECRET` outside `cron/` and `jobs/`. The only other hit is `/api/admin/env-check`, which merely *reports* whether the secret is set and is `ADMIN_API_KEY`-gated, not a job. **Wrapping `social/cron` closes the last fail-open.**
...
- **Disk space on C: — keep headroom above ~10 GB.** Below that, expect **silent corruption of written files**, not just slowness. Three occurrences now: a file truncated mid-write, then `.next` route manifests corrupting twice (symptoms differed completely — first route-group 404s where a route *compiles then renders `/_not-found`*, then 76 `tsc` errors in generated types ending in "Unterminated string literal"). **Fix both times: delete `.next`.** ⚠️ **The old reclaim list is SPENT — do not re-chase it (checked 2026-08-24).** `anaconda3`, the pip cache (~3.4 GB) and `.next` are all already gone, npm `_cacache` is down to 0.01 GB after a `npm cache clean --force` that returned only ~0.5 GB, Windows Update leftovers are 0.04 GB and `%TEMP%` 0.29 GB. **The reclaim that actually worked: `powercfg /h off`** — deletes `hiberfil.sys` (3.17 GB), took free space 8.6 → 11.8 GB in one command. Needs a genuinely **elevated** shell (`!powercfg /h off` from a Claude session does NOT work — that runs in the same non-elevated shell). Reversible with `powercfg /h on`; the only cost is losing Hibernate + Fast Startup, and no user data is involved. After that there is **no obvious next win** — no single file over 1 GB exists under the user profile, so the remaining ~100 GB is diffuse `node_modules` trees. 🔎 **Checking it: `New-Object System.IO.DriveInfo 'C'` for free space, and `Get-ChildItem 'C:\' -File -Force` to confirm a system file is gone — NOT `Test-Path`, which returns `False` on `hiberfil.sys` even while it exists (access-denied reads as "not found") and will tell you a reclaim succeeded when it failed.
...
**Queued as one security-shaped change:** absorb the duplicated cron auth into `withCronRun`. ⚠️ **The fail-open half of this item is DONE** — see the corrected §5 entry: the wrapper enforces before the handler and fails closed, so the 24 inline checks are redundant rather than dangerous. What remains is (a) deleting the dead inline blocks, (b) wrapping **`/api/social/cron`**, which is still genuinely fail-open, and (c) finishing the enumeration of `/api/jobs/*`.
