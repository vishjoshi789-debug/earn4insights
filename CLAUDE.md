...
- **⚠️ Notification preferences are enforced but UNREACHABLE — do not record them as shipped.** `dispatchToUser` genuinely consults `getPreference` before every channel, but **no page calls `/api/notifications/preferences`**, so every user is stuck on the defaults (in-app ✓ / email ✓ / SMS ✗). True in the plumbing, unreachable in practice — exactly the claim shape the §5 claims policy exists to stop.
...
**Active (v16):** **Feedback identity + the resolution loop — COMPLETE and VERIFIED IN PRODUCTION** (`ffe606b`→`1f22751`). Closes step 4 of the "real-time three-way connection": submit → brand notified → brand acts → **the consumer finds out**. Steps 1–3 were already built; step 4 had no event, no handler and no trigger.
...
- **⚠️ Notification preferences are enforced but UNREACHABLE.** `dispatchToUser` genuinely consults `getPreference` before every channel, and every event type is in the `NotifiableEventType` union — but **no page calls `/api/notifications/preferences`**, so every user sits on the defaults (in-app ✓ / email ✓ / SMS ✗) with no way to change them. **Do not record "respects notification preferences" as shipped.** Fix = a settings UI; the API and repository already exist.
