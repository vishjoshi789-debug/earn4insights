...
- ⚠️ **The route list is NOT exhaustive and the counts disagree** — §5 says 33 wrapped, §9 says 32 entries, and `/api/social/cron` is outside both. **Unfinished:** enumerate `/api/jobs/*` and confirm nothing passes `whenUnset: 'skip'`. Do not restate "all 33 routes" as fact until that is done.
