...
- **`ARCHITECTURE.md`** — Full technical architecture (authoritative). Includes "Recent Architecture Additions" appendix covering 2FA, Twilio Verify, WhatsApp feature flag, CSRF init route, LinkedIn OIDC, and Social Listening v2 Phases 1-4.
- **`docs/SCHEMA.md`** — All DB table definitions (migrations 002–020)
...
- **`docs/CRON_JOBS.md`** — Full cron schedule (32 entries), auth pattern, batch size notes. Includes `cleanup-trusted-devices` (2FA) and the env-gated `process-social-mentions` registry.
- **`docs/SOCIAL_PLATFORM_SETUP.md`** — Per-platform setup guide: status, API, cost, approval, env vars. One section per platform (Reddit, YouTube, Google, Twitter, LinkedIn, Instagram, Meta, TikTok, Amazon, Flipkart, Telegram). Future-considerations footnote on Telegram MTProto (intentionally not built).
