...
| 04:00 | `/api/cron/cleanup-trusted-devices` | Delete `trusted_devices` rows past their 30-day `expires_at` (`cleanupExpiredDevices()`). Expired rows are also pruned lazily on read in `isDeviceTrusted` — this is the sweep for devices that simply went unused. |
