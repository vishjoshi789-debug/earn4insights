...
- **`CRON_SECRET` rotation — DEFERRED, but a PRE-BETA MUST-DO (do NOT ship beta without it).** The Bearer secret used to trigger `process-deletions` during the cron test was pasted into chat → exposed. Deferred deliberately (2026-06-23): transcript is private + low harm (the cron endpoints are mostly idempotent/backstopped), so not urgent. Before beta: generate a new value → update `CRON_SECRET` in Vercel (Production) → update the Bearer in **every** cron-job.org job → redeploy → verify (new secret 200, old 401). ⚠️ if the bearer is actually `AUTH_SECRET`, prefer switching to a dedicated `CRON_SECRET` (rotating `AUTH_SECRET` invalidates all sessions).
